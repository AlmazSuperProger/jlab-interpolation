...
df=pd.read_csv('interpolation/final_table.csv', header=None, sep='\t', names=['Channel', 'MID', 'Wmin', 'Wmax', 'Q2min', 'Q2max', 'Cos(theta)', 'Sigma_T', 'dSigma_T', 'Sigma_L', 'dSigma_L', 'Sigma_TT', 'dSigma_TT', 'Sigma_LT', 'dSigma_LT', 'eps'])
...
def interpolate_in_one_region(dataframe, particle, w_min, w_max, q2_min, q2_max, w_interp='empty', q2_interp='empty', cos_interp='empty', grid_step='0.01' ):
    if str(w_interp)=='empty':
        points_num=int((w_max-w_min)/float(grid_step.replace(',','.')))
        x_axis_values=np.linspace(w_min, w_max, points_num)
        x_axis_label='W(GeV)'
        interp_df = dataframe[(dataframe.w_average>=w_min) & (dataframe.w_average<=w_max) & (dataframe.q2_average>=q2_min) & (dataframe.q2_average<=q2_max)].copy()
        sigma_TT =  griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['Sigma_TT'], (x_axis_values, q2_interp, cos_interp), method='linear', rescale=True)
        sigma_LT =  griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['Sigma_LT'], (x_axis_values, q2_interp, cos_interp), method='linear', rescale=True)
        ...
        dsigma_TT = griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_TT'], (x_axis_values, q2_interp, cos_interp), method='linear', rescale=True)
        dsigma_LT = griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_LT'], (x_axis_values, q2_interp, cos_interp), method='linear', rescale=True)
        dsigma_T =  griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_T'], (x_axis_values, q2_interp, cos_interp), method='linear', rescale=True)
        dsigma_L =  griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_L'], (x_axis_values, q2_interp, cos_interp), method='linear', rescale=True)
        ...
        dsigma_TT = griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_TT'], (w_interp, x_axis_values, cos_interp), method='linear', rescale=True)
        dsigma_LT = griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_LT'], (w_interp, x_axis_values, cos_interp), method='linear', rescale=True)
        dsigma_T =  griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
        ...
        dsigma_TT = griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_TT'], (w_interp, q2_interp, x_axis_values), method='linear', rescale=True)
        dsigma_LT = griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
                    interp_df['dSigma_LT'], (w_interp, q2_interp, x_axis_values), method='linear', rescale=True)
        dsigma_T =  griddata((interp_df.w_average, interp_df.q2_average,interp_df['Cos(theta)']),
...
def get_full_interpolation(dataframe_join, particle_join, w_interp='empty', q2_interp='empty', cos_interp='empty',grid_step='0.01' ):
    x_axis_values, sigma_TT, sigma_LT, sigma_T, sigma_L, dsigma_TT, dsigma_LT, dsigma_T, dsigma_L= [],[],[],[],[],[],[],[],[]
    if particle_join=='Pin':
        x_axis_values1, sigma_TT1, sigma_LT1, sigma_T1, sigma_L1, dsigma_TT1, dsigma_LT1, dsigma_T1, dsigma_L1 = interpolate_in_one_region(dataframe=dataframe_join,
         particle=particle_join, w_min=1.1, w_max=1.4, q2_min=0.2, q2_max=0.7, w_interp=w_interp, q2_interp=q2_interp, cos_interp=cos_interp,grid_step=grid_step )
        x_axis_values2, sigma_TT2, sigma_LT2, sigma_T2, sigma_L2, dsigma_TT2, dsigma_LT2, dsigma_T2, dsigma_L2 = interpolate_in_one_region(dataframe=dataframe_join,
         particle=particle_join, w_min=1.4, w_max=1.6, q2_min=0.2, q2_max=0.7, w_interp=w_interp, q2_interp=q2_interp, cos_interp=cos_interp,grid_step=grid_step )
        x_axis_values3, sigma_TT3, sigma_LT3, sigma_T3, sigma_L3, dsigma_TT3, dsigma_LT3, dsigma_T3, dsigma_L3 =interpolate_in_one_region(dataframe=dataframe_join,
         particle=particle_join, w_min=1.1, w_max=1.6, q2_min=1.5, q2_max=5, w_interp=w_interp, q2_interp=q2_interp, cos_interp=cos_interp,grid_step=grid_step )
        x_axis_values4, sigma_TT4, sigma_LT4, sigma_T4, sigma_L4, dsigma_TT4, dsigma_LT4, dsigma_T4, dsigma_L4 =interpolate_in_one_region(dataframe=dataframe_join,
         particle=particle_join, w_min=1.6, w_max=2.1, q2_min=1.5, q2_max=5, w_interp=w_interp, q2_interp=q2_interp, cos_interp=cos_interp,grid_step=grid_step )
    ...
    if particle_join=='Pi0P':
        x_axis_values1, sigma_TT1, sigma_LT1, sigma_T1, sigma_L1, dsigma_TT1, dsigma_LT1, dsigma_T1, dsigma_L1 = interpolate_in_one_region(dataframe=dataframe_join,
         particle=particle_join, w_min=1, w_max=1.8, q2_min=0.3, q2_max=1.9, w_interp=w_interp, q2_interp=q2_interp, cos_interp=cos_interp ,grid_step=grid_step )
        x_axis_values2, sigma_TT2, sigma_LT2, sigma_T2, sigma_L2, dsigma_TT2, dsigma_LT2, dsigma_T2, dsigma_L2 = interpolate_in_one_region(dataframe=dataframe_join,
         particle=particle_join, w_min=1, w_max=4, q2_min=2.9, q2_max=6.1, w_interp=w_interp, q2_interp=q2_interp, cos_interp=cos_interp ,grid_step=grid_step )
...
class simpleMeasure(object):
    ...
    def __init__(self, w_class='empty', q2_class='empty', cos_class='empty', e_beam_class='empty', eps_class='empty', phi_class='empty', particle_class='Pin', grid_step_class='0.01'):
        ...
        if (self.particle_class == 'Pin'):
            ...
            self.dataframe = df[(df.Channel == 8) | (df.Channel == 14) | (df.Channel == 41) | (df.Channel == 141)].copy()
        ...
        self.x_axis_values, self.sigma_TT, self.sigma_LT, self.sigma_T, self.sigma_L, self.dsigma_TT, self.dsigma_LT, self.dsigma_T, self.dsigma_L=get_full_interpolation(dataframe_join=self.dataframe,
                                             particle_join=self.particle_class, w_interp=self.w_class, q2_interp=self.q2_class, cos_interp=self.cos_class,grid_step=self.grid_step)
        ...
        try:
            if not(self.check_eps) and self.check_e_beam:
                ...
                if str(self.w_class)=='empty':
                    ...
                    self.eps_class = 1/(1 + 2*(nu*2 + self.q2_class)/(4*(self.e_beam_class - nu)*self.e_beam_class - self.q2_class))
                    
                elif str(self.q2_class)=='empty':
                    mp= 0.93827
                    nu = (self.w_class**2 + self.x_axis_values - mp*mp)/(2*mp)
                    self.eps_class = 1/(1 + 2*(nu**2 + self.x_axis_values)/(4*(self.e_beam_class - nu)*self.e_beam_class - self.x_axis_values))
        ...
        if self.check_phi and (self.check_eps or self.check_e_beam):
            ...
            self.d_res_cross_sect=(self.d_res_A**2+(self.d_res_B*np.cos(2*phi))**2+(self.d_res_C*np.cos(phi))**2)**0.5
...
def cut_structure_functions(this_object,user_choose=False, exp_data_is_available=False, this_min_val=0, this_max_val=0):
...
def create_object(q2_create, w_create, cos_create, phi_create, e_beam_create, particle_create, eps_create, grid_step_create='0.01'):
    ...
    if check_q2 and check_w and (not check_cos):
        objct= simpleMeasure(w_class=w_create, q2_class=q2_create, cos_class='empty', e_beam_class=e_beam_create, eps_class=eps_create, phi_class=phi_create, particle_class=particle_create, grid_step_class=grid_step_create)
        objct.x_label='cos(theta)'
        
        w_create=float(str(w_create).replace(',','.'))
        q2_create=float(str(q2_create).replace(',','.'))
        
        exp_df=objct.dataframe[(objct.dataframe['w_average']==w_create) & (objct.dataframe['q2_average']==q2_create)].copy()
        
        objct.exp_x=list(exp_df['Cos(theta)'])
        
        objct.exp_y=[list(exp_df['Sigma_T']), list(exp_df['Sigma_L']), list(exp_df['Sigma_TT']), list(exp_df['Sigma_LT'])]
        objct.exp_dy=[list(exp_df['dSigma_T']), list(exp_df['dSigma_L']), list(exp_df['dSigma_TT']), list(exp_df['dSigma_LT'])]
        ...
        objct=simpleMeasure(w_class=w_create, q2_class='empty', cos_class=cos_create, e_beam_class=e_beam_create, eps_class=eps_create, phi_class=phi_create, particle_class=particle_create, grid_step_class=grid_step_create)
        objct.x_label='q2(GeV2)'
        
        w_create=float(str(w_create).replace(',','.'))
        cos_create=float(str(cos_create).replace(',','.'))
        
        exp_df=objct.dataframe[(objct.dataframe['w_average']==w_create) & (objct.dataframe['Cos(theta)']==cos_create)].copy()
        ...
        objct=simpleMeasure(w_class='empty', q2_class=q2_create, cos_class=cos_create, e_beam_class=e_beam_create, eps_class=eps_create, phi_class=phi_create, particle_class=particle_create, grid_step_class=grid_step_create)
        objct.x_label='W (GeV)'
        
        cos_create=float(str(cos_create).replace(',','.'))
        q2_create=float(str(q2_create).replace(',','.'))
        
        exp_df=objct.dataframe[(objct.dataframe['Cos(theta)']==cos_create) & (objct.dataframe['q2_average']==q2_create)].copy()
...
new_clmn_names=['cos(theta)','w','q2','sigma_TT','sigma_LT', 'sigma_T', 'sigma_L', 'd_sigma_TT', 'd_sigma_LT', 'd_sigma_T', 'd_sigma_L',]  
...
if True:
    ...
    if graphObj1.x_label=='cos(theta)':
        for i in range(0,len(graphObj1.x_axis_values)):
            this_R_T, this_R_L, this_R_TT, this_R_TL, this_R_TLp, this_ds_T, this_ds_L, this_ds_TT, this_ds_TL, this_ds_TLp=maid_structure_func(W=graphObj1.w_class , Q2=graphObj1.q2_class , cos_theta=graphObj1.x_axis_values[i])
        ...
        if graphObj1.check_e_beam and graphObj1.check_phi:
            try:
                for i in range(0,len(graphObj1.x_axis_values)):
                    DCS_this=maid_cross_section(graphObj1.w_class , graphObj1.q2_class , graphObj1.x_axis_values[i], graphObj1.phi_class, graphObj1.e_beam_class, h=1)
    ...
    if graphObj1.x_label=='q2(GeV2)':
        for i in range(0,len(graphObj1.x_axis_values)):
            this_R_T, this_R_L, this_R_TT, this_R_TL, this_R_TLp, this_ds_T, this_ds_L, this_ds_TT, this_ds_TL, this_ds_TLp=maid_structure_func(W=graphObj1.w_class , Q2=graphObj1.x_axis_values[i] , cos_theta=graphObj1.cos_class)
            R_T_list.append(this_R_T)
            R_L_list.append(this_R_L)
            R_TT_list.append(this_R_TT)
            R_TL_list.append(this_R_TL)
        ...
        if graphObj1.check_e_beam and graphObj1.check_phi:
            try:
                for i in range(0,len(graphObj1.x_axis_values)):
                    DCS_this=maid_cross_section(graphObj1.w_class , graphObj1.x_axis_values[i] , graphObj1.cos_class, graphObj1.phi_class, graphObj1.e_beam_class, h=1)
                    DCS_list.append(DCS_this)
            except:
                maid_crushed=True
    ...
    if graphObj1.x_label=='W (GeV)':
        for i in range(0,len(graphObj1.x_axis_values)):
            this_R_T, this_R_L, this_R_TT, this_R_TL, this_R_TLp, this_ds_T, this_ds_L, this_ds_TT, this_ds_TL, this_ds_TLp=maid_structure_func(W=graphObj1.x_axis_values[i] , Q2=graphObj1.q2_class , cos_theta=graphObj1.cos_class)
            R_T_list.append(this_R_T)
            R_L_list.append(this_R_L)
            R_TT_list.append(this_R_TT)
            R_TL_list.append(this_R_TL)
        ...
        if graphObj1.check_e_beam and graphObj1.check_phi:
            try:
                for i in range(0,len(graphObj1.x_axis_values)):
                    DCS_this=maid_cross_section(graphObj1.x_axis_values[i] , graphObj1.q2_class , graphObj1.cos_class, graphObj1.phi_class, graphObj1.e_beam_class, h=1)
                    DCS_list.append(DCS_this)
            except:
                maid_crushed=True
...
def onePlotlyGraphWithErrors_plus_maid(x_array, y_array, maid_y_array, d_y_array, x_exp_data,y_exp_data, dy_exp_data, layout_title, x_label):
    trace = go.Scatter(
       x=x_array,
       y=y_array,
       error_y=dict(
...
if graphObj1.cant_calculate_eps:
    print('<br><br><br> ERROR: wrong E_beam for this (W,Q2,cos_theta), try to enter eps instead of the E_beam   <br><br><br>') 
...
if q2_user!='empty' and w_user!='empty':
    ...
    link_to_integrated_q2='''https://clas.sinp.msu.ru/cgi-bin/almaz/integrated_struct_function_interpolation.py?q2_user_value='''+str(link_q_value)+'''&w_user_value=&reaction_channel='''+particle_user

    link_to_integrated_w='''https://clas.sinp.msu.ru/cgi-bin/almaz/integrated_struct_function_interpolation.py?q2_user_value=&w_user_value='''+str(link_w_value)+'''&reaction_channel='''+particle_user


    print("""<br> <a href="{}">Integrated structure function W = {} (GeV) </a> <br>""".format(link_to_integrated_w, link_w_value))
    print(""" <a href="{}">Integrated structure function Q2 = {} (GeV2) </a> <br>""".format(link_to_integrated_q2, link_q_value))
...
if True:
    ...
    if cut_data_to_exp and len(graphObj1.exp_x)>1:
        ...
        print(""" <br><br><br>Interpolation:
                  ...
                  <br> interpolation points cnt ={} """.format(round(min(graphObj1.x_axis_values),3), round(max(graphObj1.x_axis_values),3), len(graphObj1.x_axis_values)))
